...
import {
  ...
  buildMonthlyGrowthRowsHSD,
  buildMonthlyGrowthRowsMS,
  calcGrowth,
  cumulativeForOutletRows,
  fiscalYearStartMonth,
  formatMonth,
  formatRoundedNumber,
  marketShareRowsAllCumulative_HSD,
...
function normalizeStationRow(r, i = 0) {
  ...
  return {
    ...
    hsd: Number(r.hsd || 0),
    hsd_ly: Number(r.hsd_ly || 0),
  };
}

function recordIdentity(record, index = 0) {
  const month = (record.month || "").toString().trim();
  const outletId = normalizeOutletId(record.outlet_id || record.id);
  if (outletId) return `${month}::id::${outletId}`;
  ...
  if (name || company || area) return `${month}::meta::${name}::${company}::${area}`;
  return `${month}::blank::${index}`;
}

function dedupeRecords(records = []) {
  const map = new Map();
  records.forEach((record, index) => {
    const normalized = normalizeStationRow(record, index);
    map.set(recordIdentity(normalized, index), normalized);
  });
...
function parseStationCsv(text) {
  ...
  Papa.parse(text, {
    ...
    skipEmptyLines: true,
    complete: ({ data }) => {
      parsedRows = dedupeRecords((data || []).map((row, index) => normalizeStationRow(row, index)));
    }
  });
  return parsedRows;
}
...
export default function FuelMapApp() {
  ...
  const [taView, setTaView] = useState({ period: "month", metric: "combined" });
  ...
  function handleCsv(file) {
    Papa.parse(file, {
      header: true, skipEmptyLines: true, complete: res => {
        const rows = dedupeRecords((res.data || []).map((r, i) => normalizeStationRow(r, i)));
        const merged = dedupeRecords([...records, ...rows]);
        setRecords(merged);
      }
    });
  }
...
{chatHistory.map((msg) => {
  ...
  return (
    ...
    <>
      ...
      {/* Right pane content */}
      {taSelected ? (
        <TradingAreaAnalysisPanel
          taSelected={taSelected}
          taView={taView}
